...
<body class="bg-[#ebf4ef] text-slate-800 antialiased min-h-screen flex flex-col justify-between selection:bg-emerald-200 selection:text-emerald-900">
    ...
    <header x-data="{ 
        ...
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2 group">
                <span class="text-xl sm:text-2xl font-extrabold tracking-tight transition-colors duration-300"
                    :class="(scrolled || !isHome || mobileMenuOpen) ? 'text-[#1b5e37] group-hover:text-[#144729]' : 'text-white drop-shadow-md'">
                    City Library
                </span>
            </a>
            ...
            <nav class="hidden md:flex items-center gap-8 text-xs sm:text-sm font-semibold transition-colors duration-300"
                ...
                <a href="{{ route('home') }}" class="transition-colors"
                    :class="(scrolled || !isHome) ? 'hover:text-[#1b5e37]' : 'hover:text-white'">Beranda</a>
                <a href="{{ route('public.books.index') }}" class="transition-colors"
                    :class="(scrolled || !isHome) ? 'hover:text-[#1b5e37]' : 'hover:text-white'">Katalog Buku</a>
                <a href="{{ route('home') }}#kategori" class="transition-colors"
                    :class="(scrolled || !isHome) ? 'hover:text-[#1b5e37]' : 'hover:text-white'">Kategori Buku</a>
                <a href="{{ route('home') }}#faq" class="transition-colors"
                    :class="(scrolled || !isHome) ? 'hover:text-[#1b5e37]' : 'hover:text-white'">FAQ</a>
            </nav>
            ...
            <div class="flex items-center gap-3">
                ...
                @else
                    <a href="{{ route('login') }}"
                        ...
                        :class="(scrolled || !isHome) ? 'text-slate-700 hover:text-[#1b5e37]' : 'text-white hover:text-emerald-200 drop-shadow-xs'">
                        Masuk
                    </a>
                ...
                <div class="w-8 h-8 rounded-full border flex items-center justify-center transition-colors cursor-pointer"
                    :class="(scrolled || !isHome) ? 'border-slate-200 bg-white text-slate-500 hover:text-[#1b5e37] hover:border-emerald-300' : 'border-white/30 bg-white/20 backdrop-blur-md text-white hover:bg-white/30'"
                ...
                <button type="button" @click="mobileMenuOpen = !mobileMenuOpen"
                    ...
                    :class="(scrolled || !isHome || mobileMenuOpen) ? 'text-slate-700 hover:bg-emerald-100/50' : 'text-white hover:bg-white/20'"
                    title="Toggle menu">
        ...
        <div x-show="mobileMenuOpen" x-cloak @click.outside="mobileMenuOpen = false"
            ...
            <a href="{{ route('home') }}" @click="mobileMenuOpen = false" class="px-3 py-2 rounded-lg hover:bg-emerald-100/60 hover:text-[#1b5e37] transition-colors">Beranda</a>
            <a href="{{ route('public.books.index') }}" @click="mobileMenuOpen = false" class="px-3 py-2 rounded-lg hover:bg-emerald-100/60 hover:text-[#1b5e37] transition-colors">Katalog Buku</a>
            <a href="{{ route('home') }}#kategori" @click="mobileMenuOpen = false" class="px-3 py-2 rounded-lg hover:bg-emerald-100/60 hover:text-[#1b5e37] transition-colors">Kategori Buku</a>
            <a href="{{ route('home') }}#faq" @click="mobileMenuOpen = false" class="px-3 py-2 rounded-lg hover:bg-emerald-100/60 hover:text-[#1b5e37] transition-colors">FAQ</a>
            @guest
                <a href="{{ route('login') }}" @click="mobileMenuOpen = false" class="px-3 py-2 rounded-lg hover:bg-emerald-100/60 text-[#1b5e37] font-bold transition-colors">Masuk ke Akun</a>
            @endguest
        </div>
    </header>
