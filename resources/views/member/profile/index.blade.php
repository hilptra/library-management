...
<div class="space-y-6 pt-2 max-w-5xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 text-xs sm:text-sm font-medium mt-0.5">Informasi akun dan status keanggotaan perpustakaan Anda</p>
        </div>
        <div>
    ...
    <div class="bg-white rounded-2xl p-6 lg:p-8 shadow-xs border border-slate-100/90 relative overflow-hidden">
        ...
        <div class="absolute -right-10 -bottom-10 w-48 h-48 bg-emerald-50 rounded-full opacity-60 pointer-events-none"></div>

        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
            <div class="flex items-center gap-5">
                {{-- Avatar Circle --}}
                <div class="relative">
                    <div class="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-gradient-to-br from-[#409a63] to-[#2d6e46] text-white text-2xl sm:text-3xl font-extrabold flex items-center justify-center shadow-md border-4 border-emerald-50">
                        {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                    </div>
                    <span class="absolute -bottom-1 -right-1 w-5 h-5 bg-emerald-500 border-2 border-white rounded-full flex items-center justify-center" title="Akun Aktif">
                ...
                <div class="space-y-1">
                    <div class="flex items-center gap-2.5 flex-wrap">
                        ...
                        <span class="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-xs font-bold bg-[#dcfce7] text-[#166534] border border-emerald-200/60">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-xs border border-slate-100/90 flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-extrabold text-slate-900 mt-1">{{ $totalLoans }} <span class="text-xs font-medium text-slate-500">Buku</span></h3>
            </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-xs border border-slate-100/90 flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-extrabold text-slate-900 mt-1">{{ $activeLoans }} <span class="text-xs font-medium text-slate-500">Buku</span></h3>
            </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-xs border border-slate-100/90 flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-extrabold text-slate-900 mt-1">{{ $returnedLoans }} <span class="text-xs font-medium text-slate-500">Buku</span></h3>
            </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-xs border border-slate-100/90">
                <div class="flex items-center justify-between pb-4 mb-6 border-b border-slate-100">
                    ...
                    <a href="{{ route('member.profile.edit') }}" class="text-xs font-bold text-[#409a63] hover:text-[#348353]">
                        Ubah Informasi &rarr;
                    </a>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Nomor Telepon / WA</p>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Tanggal Bergabung</p>
                            <p class="text-slate-800 font-bold mt-0.5">{{ Auth::user()->created_at->format('d F Y') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Status Keanggotaan</p>
            ...
            <div class="bg-gradient-to-r from-emerald-900 to-slate-800 rounded-2xl p-6 text-white shadow-xs relative overflow-hidden">
                <div class="absolute -right-6 -top-6 w-32 h-32 bg-emerald-500/10 rounded-full blur-xl pointer-events-none"></div>
                <div class="flex items-start gap-4 relative z-10">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-slate-300 text-xs leading-relaxed">
                            Pastikan nomor telepon dan data diri Anda selalu diperbarui agar staf perpustakaan dapat menghubungi Anda mengenai konfirmasi peminjaman buku maupun pengingat masa pinjam.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-xs border border-slate-100/90 space-y-4">
                <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                    <h3 class="text-base font-bold text-slate-900 flex items-center gap-2">
                        <svg class="w-5 h-5 text-[#409a63]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" />
                        </svg>
                        <span>Kartu Anggota Digital</span>
                    </h3>
                    <span class="text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full">Resmi</span>
                </div>

                {{-- Visual Card Widget --}}
                <div class="bg-gradient-to-br from-[#166534] via-[#228448] to-[#409a63] rounded-2xl p-5 text-white shadow-lg relative overflow-hidden border border-emerald-400/20">
                    {{-- Decorative Patterns --}}
                    <div class="absolute -right-8 -bottom-8 w-36 h-36 bg-white/10 rounded-full blur-xs pointer-events-none"></div>
                    <div class="absolute top-0 right-0 w-24 h-24 bg-emerald-300/10 rounded-bl-full pointer-events-none"></div>

                    {{-- Top Header Card --}}
                    <div class="flex items-center justify-between mb-6 relative z-10">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-xl bg-white/20 backdrop-blur-md flex items-center justify-center font-extrabold text-xs text-white">
                                PK
                            </div>
                            <div>
                                <h4 class="font-extrabold text-xs tracking-wide uppercase leading-none">Perpustakaan Kota</h4>
                                <p class="text-[9px] text-emerald-200 mt-0.5">Kartu Anggota Perpustakaan</p>
                            </div>
                        </div>
                        <span class="text-[10px] font-mono tracking-widest text-emerald-200 bg-black/20 px-2 py-1 rounded">MEMBER</span>
                    </div>

                    {{-- Card Body --}}
                    <div class="space-y-3 relative z-10">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 rounded-xl bg-white/15 backdrop-blur-sm border border-white/20 text-white font-bold text-base flex items-center justify-center shrink-0">
                                {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                            </div>
                            <div class="overflow-hidden">
                                <h3 class="font-extrabold text-sm truncate leading-tight">{{ Auth::user()->name }}</h3>
                    ...
                    {{-- Card Footer & Code --}}
                    <div class="mt-6 pt-3 border-t border-white/15 flex items-end justify-between relative z-10 text-[10px]">
                        <div>
                            <p class="text-emerald-200 uppercase tracking-wider text-[8px]">ID Anggota</p>
                            <p class="font-mono font-bold text-xs text-white">LIB-MBR-{{ str_pad(Auth::user()->id, 5, '0', STR_PAD_LEFT) }}</p>
                        </div>
