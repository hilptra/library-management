    ...
    <div class="space-y-6 pt-2">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-slate-500 text-xs sm:text-sm font-medium mt-0.5">Kelola transaksi dan riwayat peminjaman buku
                </p>
            </div>
        </div>
                ...
                <table class="w-full text-left text-sm border-collapse">
                    <thead>
                        <tr class="text-[11px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100">
                            ...
                            @if ($status === 'borrowed')
                                <th class="pb-3 px-4">Estimasi Denda</th>
                        ...
                        @forelse ($loans as $loan)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="py-3.5 px-4 font-mono text-xs text-slate-600">
                                    {{ $loan->bookCopy->inventory_code ?? '-' }}</td>
                                <td class="py-3.5 px-4 text-slate-500 text-xs">
                                    {{ $loan->loan_date?->format('d M Y') ?? '-' }}</td>
                                @if ($status === 'returned')
                                    <td class="py-3.5 px-4 text-slate-500 text-xs">
                                        {{ $loan->return_date?->format('d M Y') ?? '-' }}</td>
                                @endif
                                <td class="py-3.5 px-4 text-slate-500 text-xs">
                                    {{ $loan->due_date?->format('d M Y') ?? '-' }}</td>
                                @if ($status === 'borrowed')
                                ...
                                @elseif ($status === 'returned')
                                    <td class="py-3.5 px-4 text-xs font-semibold">
                                        @if (($loan->fine_amount ?? 0) > 0)
                                            <span class="text-rose-600 font-bold">Rp
                                                {{ number_format($loan->fine_amount, 0, ',', '.') }}</span>
                                        @else
                                            <span class="text-slate-400">Rp 0</span>
                                        @endif
                                    </td>
                                ...
                                @if ($status === 'pending')
                                    <td class="py-3.5 px-4 text-right space-x-2">
                                        <form action="{{ route('admin.loans.approve', $loan) }}" method="POST"
                                            ...
                                            @method('PATCH')
                                            <button type="submit"
                                                class="bg-[#409a63] hover:bg-[#348353] text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-2xs transition-colors">
                                                Setujui
                                            </button>
                                        </form>
                                        <form action="{{ route('admin.loans.reject', $loan) }}" method="POST"
                                            ...
                                            @method('PATCH')
                                            <button type="submit"
                                                class="bg-rose-50 text-rose-700 hover:bg-rose-100 text-xs font-bold px-3 py-1.5 rounded-lg border border-rose-200 transition-colors">
                                                Tolak
                                            </button>
                                        </form>
                                    </td>
                        ...
                        @empty
                            <tr>
                                @php
                                    $colCount =
                                        5 +
                                        (in_array($status, ['borrowed', 'returned']) ? 1 : 0) +
                                        (in_array($status, ['pending', 'borrowed']) ? 1 : 0);
                                @endphp
                                <td colspan="{{ $colCount }}" class="py-8 text-center text-slate-400 font-medium">Tidak
