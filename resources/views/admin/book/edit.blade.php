...
<div class="max-w-3xl mx-auto space-y-6 pt-2">
    ...
    <div class="bg-white rounded-2xl p-6 sm:p-8 shadow-xs border border-slate-100/90">
        <form action="{{ route('books.update', $book) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Cover Saat Ini</label>
                @if ($book->cover_image)
                    <img src="{{ asset('storage/' . $book->cover_image) }}" class="w-24 h-32 object-cover rounded-xl mb-2 shadow-2xs border border-slate-100">
                @else
